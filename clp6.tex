...
meghiúsulása).
\br
\definicio \emph{Megjelenítõ}nek (\emph{visualizer}) nevezzük a \clpfd eseményekre
...
nyomkövetést az \cd{fdbg\_output} álnevû (stream alias) folyamra írja a
...
\item \cd{file({\em Filename}, {\em Mode})}\\
  A megjelenítõk kimenete a \cd{\em Filename} nevû állományba irányítódik
...
\item \cd{stream({\em Stream})}\\
  A megjelenítõk kimenete a \cd{\em Stream} folyamra irányítódik át.
\item \cd{constraint\_hook({\em Goal})}\\
  \cd{\em Goal} két argumentummal kiegészítve meghívódik a korlátok
  felébredésekor. Alapértelmezésben ez az \cd{fdbg\_show/2} eljárás, ld.~késõbb.
...
\item \cd{labeling\_hook({\em Goal})}\\
  ...
  eseménykor. Alapértelmezésben ez az \cd{fdbg\_label\_show/3}, ld.~késõbb.
...
Kimenet átirányítása, beépített megjelenítõ, nincs címkézési nyomkövetés:
...
az \cd{<fdvar_{\em x}>} forma helyett fog megjelenni. Lehetõség van arra is,
...
\item \bcd{fdbg\_get\_name(+{\em Term}, -\em{Name})} \\
    ...
    neve, automatikusan hozzárendelõdik egy.
}

Pl.~\cd{fdbg\_assign_name(foo, bar(A, [B, C]))} hatására a következõ nevek
generálódnak:
...
rendelkezésére. A korlát-megjelenítõt az \cd{fdbg_show/2}, a címkézés-megjelenítõt
...
Beépített korlát-megjelenítõ. A \cd{dispatch_global}-ból való kilépéskor hívódik
...
,,Szimulált'' példa-hívás \cd{X1=3} és \cd{X2=3} szûkítésekkel:
...
pedig magát a címkézendõ változót. \cd{\em Event} megegyezik a
...
\subsection{Testreszabás kampó-eljárásokkal}
...
{\em Actions})}\\
\cd{\em my\_global\_visualizer} helyére tetszõleges predikátumnév kerülhet, \cd{\em Arg1},
...
(az utolsó kettõ kivételével) az \cd{fdbg_on} opciólistájában kell átadni:\\
...
{\em ID}, {\em Var})}\\
\cd{\em Event} a megjelenítõ meghívását kiváltó címkézési esemény, a következõek szerint:
\begin{itemize}
...
Érdemes egy pillantást vetni az \cd{fdbg\_show/2} beépített megjelenítõ kódjára is,
...
\item \bcd{fdbg\_annotate(+{\em Term0}, -{\em Term}, -{\em Vars})} \\
  ...
  \item egy FD halmaz, amely a változó tartománya \emph{lesz} az
    \cd{\em Actions} akciólista szûkítései után.
  \end{itemize}
...
\cd{portray/1} megjelenítõ eljárást, ezért ha a \cd{print/1} használatával
...
\item \bcd{fdbg\_legend(+{\em Vars})} \\
  ...
  Az \cd{fdbg\_annotate/3,4} használatával elõálló \cd{\em Vars} változólistát
...
\subsection{A mágikus sorozatok feladat nyomkövetése}
